...
# removing monogenic diabetes genes
  ...
  if (trait == "FG") {
    ...
    test[[d]] = read.table(
      file = paste(
        ...
        "/2019-10-06_physlociDimasWood_T2D_annotated_genes_in_credible_regions_plusminus_",
        d,
        "_kb.txt",
        sep = ""
      ),
...
for(d in distance){
  ...
  if (type=="all"){
    write.table(df_list[[d]],quote=F,row.names=T,col.names = T,file=paste("/Users/Marta/Documents/WTCHG/R scripts/Diff_v2/Thesis/Genes_in_credible_regions_T2D_HRC_2018/",
                                                                          ...
                                                                          d,"_",
                                                                          "_kb_from_credible_regions_physloci.txt",sep=""),sep="\t")
...
### without monogenic diabetes genes


trait = "T2D"    # T2D or fasting glucose (FG)?
type = "all"     #or top623
order = "p-value"   # or logFC
in_folder_DE = "/Users/Marta/Documents/WTCHG/DPhil/Data/Results/Diff_v2/Voom/across-stages/2017-07-03_results_without_monogenic_0kb"  # folder that contains diff expression data 
# removing monogenic diabetes genes
